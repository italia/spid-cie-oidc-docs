...
.. list-table:: 
   ...
   * - **Claim**
     ...
     - **Supported by**
   * - **client_assertion**
     - REQUIRED. JWT signed with the Relying Party's private key, containing the same parameters as documented 
   ...
   * - **client_assertion_type**
     - REQUIRED. String. Allowed values: **urn:ietf:params:oauth:clientassertion-type:jwt-bearer**
     - |spid-icon| |cieid-icon| 
   * - **client_id**
     - REQUIRED. URI that unquely identifies the RP. The OP must check that the client_id is known inside the 
       Federation.
     - |spid-icon| |cieid-icon| 
   * - **token**
     - REQUIRED. The token about which the RP wants to obtain information.
     - |spid-icon| |cieid-icon|  
...
.. list-table:: 
   ...
   * - **Claim**
     ...
     - **Supported by**
   * - **active**
     - REQUIRED. Boolean value that indicates the token validity. If the token is expired, it has been revoked or it
