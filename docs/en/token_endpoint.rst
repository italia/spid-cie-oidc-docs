...
.. list-table:: 
   ...
   * - **jti** 
     - String. Unique identifier of the ID Token that the client can use for preventing a reuse, refusing the
       ID Token if it has already been processed. It must be difficult to guess by an attacker and composed of a casual string. It is suggested to use the *uuid4* generator.
     - |spid-icon| |cieid-icon|
   * - **nonce** 
     - String. Casual string generated by the RP for each user's session and sent in the 
