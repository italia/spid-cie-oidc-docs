...
.. list-table:: 
   ...
   * - **client_assertion**
     - REQUIRED. JWT signed with the Relying Party's private key containing the following parameters:
	 
	 **iss**: REQUIRED. This MUST contain the *client_id*.
	 
	 **sub**: REQUIRED. This MUST contain the *iss*.
	 
	 **aud**: REQUIRED. URL of the OP Token Endpoint.
	 
	 **iat**: REQUIRED. UNIX Timestamp with the time of the JWT issuance, coded as NumericDate as indicated in :rfc:`7519`. 
	 
	 **exp**: REQUIRED. UNIX Timestamp with the expiry time of the JWT, coded as NumericDate as indicated in :rfc:`7519`. 
	 
	 **jti**: REQUIRED. Unique Identifier uuid4 for this authentication request, generated by the client. Eg: it can be in *uuid4* format.
     - |spid-icon| |cieid-icon|
   * - **client_assertion_type**
     - REQUIRED. It must get the following value: |br|
       **urn:ietf:params:oauth:client-assertion-type:jwt-bearer**.
     - |spid-icon| |cieid-icon|
   * - **code**
     - REQUIRED WHEN **grant_type** is **authorization_code**. Authorization code returned in the Authentication Response.
     - |spid-icon| |cieid-icon|
   * - **code_verifier**
     - REQUIRED WHEN **grant_type** is **authorization_code**. Verification code of the code_challenge.
     - |spid-icon| |cieid-icon|
   * - **grant_type**
     - REQUIRED. Type of credentials, presented by the RP, for the current request.
...
.. list-table:: 
   ...
   * - **Claim**
     ...
     - **Supported by**
   * - **access_token**
     - REQUIRED. The Access Token, in signed JWT format, allows accessing the UserInfo Endpoint for obtaining the attributes.
     - |spid-icon| |cieid-icon|
   * - **token_type**
     - REQUIRED. Type of *Access Token* returned. It MUST always be equal to **Bearer**.
     - |spid-icon| |cieid-icon|
   * - **refresh_token**
     - REQUIRED WHEN a `long revocable session`_ is available. The *Refresh Token* MUST be a signed JWT format. It allows calling again the *Token Endpoint* for obtaining a new *Access Token* and a new *ID Token*.
     - |spid-icon| |cieid-icon|
   * - **expires_in**
     - REQUIRED. Expity time of the *Access Token* in seconds.
     - |spid-icon| |cieid-icon|
   * - **id_token**
     - REQUIRED. ID Token in JWT format (see next section).
     - |spid-icon| |cieid-icon|
...
.. list-table:: 
   ...
   * - **Claim**
     ...
     - **Supported by**
   * - **iss** 
     - REQUIRED. It MUST be an HTTPS URL that uniquely identifies the OP. The client MUST verify that this value matches the called OP.
     - |spid-icon| |cieid-icon|
   * - **sub** 
     - REQUIRED. See `OpenID.Core#SubjectIDTypes`_. It MUST be of type *pairwise*. 
     - |spid-icon| |cieid-icon|
   * - **client_id** 
     - REQUIRED. It MUST contain a HTTPS URL that uniquely identifies the RP. 
     - |spid-icon| |cieid-icon|
   * - **aud** 
     - REQUIRED. It MUST contain a list of Resource Servers referring to token consuming party. It MUST contain at least the *UserInfo Endpoint*. 
     - |spid-icon| |cieid-icon|
   * - **scope** 
     - REQUIRED. The OP SHOULD add the *scope* parameter as defined in :rfc:`9068` Section 2.2.3. It MUST match the value in the authentication request.
     - |spid-icon| |cieid-icon|
   * - **iat** 
   ...
   * - **exp**
     ...
     - |spid-icon| |cieid-icon|
   * - **jti** 
     - REQUIRED. It MUST be a String in *uuid4* format. Unique Token ID identifier that the RP MAY use to prevent reuse by rejecting the Token ID if already processed.
     - |spid-icon| |cieid-icon|
   * - **nonce** {{TO_VERIFY, not present in the italian version}}
...
.. list-table:: 
   ...
   * - **iss** 
     ...
     - |spid-icon| |cieid-icon|
   * - **sub** 
     - REQUIRED. See `OpenID.Core#SubjectIDTypes`_. It MUST be *pairwise* type. 
     - |spid-icon| |cieid-icon|
   * - **aud** 
     - REQUIRED. It MUST match the value *client_id*. The RP MUST verify that this value matches its client ID.
     - |spid-icon| |cieid-icon|
   * - **acr** 
     - REQUIRED. Effective authentication level. It MUST be equal or greater than the one requested by the client in the Authentication Request.
     - |spid-icon| |cieid-icon|
   * - **at_hash** 
   ...
   * - **iat** 
     ...
     - |spid-icon| |cieid-icon|
   * - **nbf** 
     - REQUIRED. UNIX Timestamp. Time of the validity beginning of the JWT in NumericDate format, as indicated in :rfc:`7519`. MUST match with the value of **iat**.
     - |spid-icon| |cieid-icon|
   * - **exp**
     - REQUIRED. UNIX Timestamp with the expiry time of the JWT, coded as NumericDate as indicated in :rfc:`7519`.
     - |spid-icon| |cieid-icon|
   * - **jti** 
     ...
     - |spid-icon| |cieid-icon|
   * - **nonce** 
     - REQUIRED. See `OpenID.Core#AuthRequest`_. It MUST be a random string of at least 32 alphanumeric characters. This value MUST match the value sent by the RP in the authentication request.
     - |spid-icon| |cieid-icon|
...
.. list-table:: 
  ...
  * - **Claim**
    ...
    - **Supported by**
  * - **iss** 
    - REQUIRED. It MUST be an HTTPS URL that uniquely identifies the OP. The RP MUST verify that this value matches the called OP.
    - |spid-icon| |cieid-icon|
  * - **client_id** 
    - REQUIRED. It MUST match the value client_id. The RP MUST verify that this value matches its client ID.
    - |spid-icon| |cieid-icon|
  * - **aud** 
    - REQUIRED. It MUST contain the OP *Token Endpoint*.
    - |spid-icon| |cieid-icon|
  * - **iat** 
    - REQUIRED. UNIX Timestamp with the time of JWT issuance, coded as NumericDate as indicated in :rfc:`7519`.
    - |spid-icon| |cieid-icon|
  * - **exp** 
    ...
    - |spid-icon| |cieid-icon|
  * - **jti** 
    - REQUIRED. It MUST be a String in uuid4 format. Unique *Refresh Token* identifier that the RP MAY use to prevent reuse by rejecting the *Refresh Token*  if already processed.
    - |spid-icon| |cieid-icon|
