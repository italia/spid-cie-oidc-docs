...
.. list-table:: 
  ...
  * - **Claim**
    ...
    - **Supported by**
  * - **scope**
    - REQUIRED. It contains values of *scope* that are supported by the OP and defined by the parameter
      **scopes_supported** in the :ref:`Metadata OP <MetadataOP>`. 
      At least the value *openid* MUST be present.
    - |spid-icon| |cieid-icon|
  * - **code_challenge**
    - REQUIRED. See :rfc:`7636#section-4.2`.
    - |spid-icon| |cieid-icon|
  * - **code_challenge_method**
    - REQUIRED. As defined by the parameter **code_challenge_methods_supported** in the :ref:`Metadata OP <MetadataOP>`.
    - |spid-icon| |cieid-icon|
  * - **request**
    - REQUIRED. See `OpenID.Core#JWTRequests`_. It MUST be a signed **JWT**.
    - |spid-icon| |cieid-icon|
...
.. list-table:: 
  ...
  * - **Jose Header**
    ...
    - **Supported by**
  * - **alg**
    - REQUIRED. See :rfc:`7516#section-4.1.1`. See :ref:`supported_algs`.
    - |spid-icon| |cieid-icon|
  * - **kid**
...
.. list-table:: 
   ...
   * - **Claim**
     ...
     - **Supported by**
   * - **client_id**
     - REQUIRED. See `OpenID.Registration`_. It MUST contain an HTTPS URL that uniquely identifies the RP.
     - |spid-icon| |cieid-icon|
   * - **code_challenge**
     - REQUIRED. As defined in the :ref:`Table of the HTTP parameters <tabella_parametri_http_req>`.
     - |spid-icon| |cieid-icon|
   * - **code_challenge_method**
     - REQUIRED. As defined in the :ref:`Table of the HTTP parameters <tabella_parametri_http_req>`.
     - |spid-icon| |cieid-icon|
   * - **nonce**
     - REQUIRED. See `OpenID.Core#AuthRequest`_. It MUST be a casual string with at least 32 alphanumeric characters.
     ...
     - |spid-icon| |cieid-icon|
   * - **prompt**
     - REQUIRED. See `OpenID.Core#AuthRequest`_. The allowed values are:
     ...
     - |spid-icon| |cieid-icon|
   * - **redirect_uri**
     - REQUIRED. See `OpenID.Core#AuthRequest`_. It MUST be an URL included in the :ref:`Metadata RP <MetadataRP>`. 
     - |spid-icon| |cieid-icon|
   * - **response_type**
     - REQUIRED. See `OpenID.Core#AuthRequest`_. As defined by the parameter **response_types_supported** in the
   ...
   * - **scope**
     ...
     - |spid-icon| |cieid-icon|
   * - **acr_values**
     - REQUIRED. See `OpenID.Core#AuthRequest`_. As defined by the parameter **acr_values_supported** in the
       :ref:`Metadata OP <MetadataOP>`.
     ...
     - |spid-icon| |cieid-icon|
   * - **claims**
     - REQUIRED. See `OpenID.Core#ClaimsRequestParameter`_. See Section "Parameters scope and claims".
     - |spid-icon| |cieid-icon|
   * - **state**
     - REQUIRED. See `OpenID.Core#AuthRequest`_. It must be a casual string with at least 32 alphanumeric characters.
   ...
   * - **exp**
     ...
     - |spid-icon| |cieid-icon|
   * - **iat**
     - REQUIRED. UNIX Timestamp with the generation time of the JWT, coded as NumericDate as indicated in :rfc:`7519`
     - |spid-icon| |cieid-icon|
   * - **iss**
     - REQUIRED. It MUST correspond to *client_id*. 
     - |spid-icon| |cieid-icon|
   * - **aud**
     - REQUIRED. It MUST correspond to the OP identifier (parameter *issuer*, present in the :ref:`Metadata OP <MetadataOP>`.)
     - |spid-icon| |cieid-icon|
   * - **ui_locales**
...
.. list-table:: 
   ...
   * - **Claim**
     ...
     - **Supported by**
   * - **code**
     - REQUIRED. Unique *Authorization Code* that the client can pass to the Token Endpoint for obtaining an ID Token and an Access Token. This has the advantage of not exposing any token to the User Agent or to malware that could be controlling it.
     - |spid-icon| |cieid-icon|
   * - **state**
     - REQUIRED. State value included in the *Authentication Request*. The client is supposed to check its correspondence. It must have the same value indicated by the client in the Authorization Request.
     - |spid-icon| |cieid-icon|
   * - **iss**
     - REQUIRED. Unique Identifier of the OP that has created the Authentication Response. The RP MUST validate
...
.. list-table:: 
   ...
   * - **Claim**
     ...
     - **Supported by**
   * - **error**
     - REQUIRED. See :ref:`Error codes <codici_errore>`
     - |spid-icon| |cieid-icon|
   * - **error_description**
     - REQUIRED. Error description.
     - |spid-icon| |cieid-icon|
   * - **state**
     - REQUIRED WHEN it is an error response to the *Authentication Request*. It MUST be equal to the *status* value included in the *Authentication Request*. The RP MUST verify that it matches the one sent in the *Authentication Request*.
     - |spid-icon| |cieid-icon|
