...
Differences between SPID e CIE id profiles
------------------------------------------
...
In CIE id, the OP and RP metadata include the parameters that enable the ID Token encryption (see section `Metadata OP <MetadataOP>`_ and `Metadata RP <MetadataRP>`_)

SPID does not allow the ID Token encryption, thus such a parameters are not required. 
...
In SPID profile the user attributes are not available in the ID Token. 

In CIE id they are available in the ID Token and at the UserInfo endpoint. 
...
CIE id profile supports both GET and POST HTTP methods. 

SPID profile only allows HTTP GET method.
...
A Refresh Token revocation requires all related active token to be revoked.  
