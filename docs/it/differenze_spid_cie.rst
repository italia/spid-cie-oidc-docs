...
Differenze tra i profili SPID e CIE id
--------------------------------------
...
Inoltre, il metadata OP per CIE id richiede anche il parametro *revocation_endpoint_auth_methods_supported* che non è richiesto da SPID.
...
Inoltre, il profilo CIE id prevede come obbligatorio il parametro *iss* nella response per evitare l'attacco mix-up `I-D.ietf-OAuth-Security-BCP`_.


Parametri Scope e Claims
++++++++++++++++++++++++

Il profilo CIE id consente di richiedere gli attributi dell'utente sia tramite il parametro *claims* nella richiesta di autenticazione e sia tramite il parametro *scope* abilitando i valori *profile* e *email*. 
...
Il profilo SPID non consente di rilasciare gli attributi dell'utente all'interno dell'ID Token. 
In CIE id gli attributi dell'utente sono disponibili sia nell'ID Token e sia allo UserInfo endpoint. Inoltre, il profilo CIE id supporta la cifratura dell'ID Token.
...
Il profilo CIE id supporta entrambi i metodi HTTP GET e HTTP POST per le richieste allo UserInfo endpoint.
Il profilo SPID consente solo l'utilizzo del metodo HTTP GET.
